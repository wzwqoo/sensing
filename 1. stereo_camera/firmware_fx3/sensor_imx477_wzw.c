...
static imx477_reg_t mode_640x480_regs[] = {
		...
		{REG_CSI_LANE, 0x03}, // wzw: change to 0x03 for 4 lane csi2
		{REG_LINE_LEN_MSB, 0x1a},
		{REG_LINE_LEN_LSB, 0x08}, // The sensor spends 6664 clocks processing one line.
		{REG_FRAME_LEN_MSB, 0x04},
		{REG_FRAME_LEN_LSB, 0x1a}, // It repeats this process 1050 times to complete one frame.
		...
		{0xa2a9, 0x27},
		{0xa2b7, 0x03},
		{REG_SCALE_MODE, 0x00}, // scaling off
		{REG_SCALE_M_MSbit, 0x00},
		{REG_SCALE_M_LSB, 0x10}, // Sony sensors typically use a scaling formula where the Factor = 16/M
...
void sensor_configure_mode(imgsensor_mode_t * mode)
{
	...
	// Frame timing - CRITICAL for proper video timing
	sensor_i2c_write(REG_LINE_LEN_MSB, (mode->line_length & 0xFF00)>>8);
	sensor_i2c_write(REG_LINE_LEN_LSB,  mode->line_length & 0xFF);
	sensor_i2c_write(REG_FRAME_LEN_MSB, (mode->frame_length & 0xFF00)>>8);
	sensor_i2c_write(REG_FRAME_LEN_LSB,  mode->frame_length & 0xFF);
	// test pattern is off here
	...
	// Exposure - needed for proper image brightness, This value must be less than REG_FRAME_LEN
...
void SensorInit (void){
	...
	{
		//CyU3PDebugPrint (4, "Reg 0x%x val 0x%x\n", (mode_default + i)->address, (mode_default + i)->val);
		sensor_i2c_write((mode_default + i)->address, (mode_default + i)->val); //// write default register value to register address
	}
