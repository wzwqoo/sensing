...
static bool glHitEOF = false; // To fix this, when FULL_BUF_IN_SCK0 happens, the firmware should set a global flag (e.g., glHitEOF = true).
 ...
 */
static void CyFxUVCApplnAbortHandler (
        void)
{
...
/* This is the Callback function to handle the USB Events, high level events like connect/disconnect */
        ...
        )
{
    ...
    switch (bmReqType)
    {
        ...
        case CY_FX_USB_UVC_SET_REQ_TYPE:
            // Defer processing to another thread via events
        ...
        )
{
    ...
    if (type == CY_U3P_DMA_CB_PROD_EVENT)
    {
        ...
        while (status == CY_U3P_SUCCESS)
        {
            ...
            /* Add Headers*/
            if (glHitEOF) {
                /* Set the EOF bit in the UVC header */
                ...
                glHitEOF = false; // reset for next frame
            }
            else {
...
static void CyFxUVCApplnDebugInit (void)
{
    ...
    uartConfig.flowCtrl = CyFalse; // Disable CTS/RTS
...
static void CyFxUvcAppGpifInit (void)
{
    ...
    // wzw: Take the massive array located inside cyfxgpif2config.h and load it into the GPIF II engine.
 ...
 */
static CyBool_t CyFxUVCAppLPMRqtCB (CyU3PUsbLinkPowerMode link_mode) {/*USB 3.0 linkmode requested by Host */
    return CyTrue;
}
...
static void CyFxUVCApplnInit (void){
    ...
    /* GPIO Step B: Initialize the GPIO Block */
    ...
    /* GPIO Step C: Configure the Specific Pins (Direction & Initial Value) */
    ...
    * must use GpioOverride to configure it */
    ...
    // determining the speed and timing of the data interface connecting to external components like an FPGA
    pibclock.clkDiv      = 2;
    pibclock.clkSrc      = CY_U3P_SYS_CLK;
    pibclock.isDllEnable = CyFalse;
    pibclock.isHalfDiv   = CyFalse;

    // wzw: Before loading the state machine, you must turn on the physical parallel interface.
    // Initialize the Parallel IO Block
...
static void UVCHandleProcessingUnitRqts (void){
    ...
    // Saturation Control is actually Exposure: When the host sends a CY_FX_UVC_PU_SATURATION_CONTROL request (e.g., the user moves the "Saturation" slider in OBS or Windows Camera), the code calls sensor_set_exposure().
    // Gamma Control is actually Test Pattern: When the host sends a CY_FX_UVC_PU_GAMMA_CONTROL request, the code calls sensor_set_test_pattern().
    switch (wValue)
    {
            ...
            switch (bRequest)
            {
                ...
                case CY_FX_USB_UVC_SET_CUR_REQ:
                    ...
                    if (apiRetStatus == CY_U3P_SUCCESS)
                    {
                        ...
                        sensor_handle_uvc_control(uvc_control->frame_interval);
...
void UVCAppEP0Thread_Entry (uint32_t input){
    ...
    for (;;)
    {
        /* Wait forever until a Video control or streaming related request on the control endpoint. */
...
int main (void)
{
    ...
    /* step A0: Initialize the system clock */
    clk_cfg.setSysClk400  = CyTrue; // default is false, 19.2*21=403.2
    clk_cfg.cpuClkDiv     = 2;
    clk_cfg.dmaClkDiv     = 2;
    clk_cfg.mmioClkDiv    = 2;
    clk_cfg.useStandbyClk = CyFalse; // default is true
    clk_cfg.clkSrc        = CY_U3P_SYS_CLK; // no division
    ...
    /* GPIO step A: Configure the IO matrix for the device. */
    io_cfg.isDQ32Bit        = CyTrue; // combination of isDQ32Bit and lppMode decides uart pin location, i2c pins fixed
